...
- (void)show
{
    if (! self.visible)
    {
		...
		[[self alertView] addGestureRecognizer:singleFingerTap];
...
- (void)repositionControls
{
    ...
    if (alertView.numberOfButtons > 0)
    {
        ...
        self.alertView.frame = (CGRect){{self.alertView.frame.origin.x, self.alertView.frame.origin.y}, {self.alertView.frame.size.width, self.alertView.frame.size.height + 40.f}};
    }
	//[self moveFrameDownBy:40.0f];
}

- (void)moveFrameDownBy: (float_t)  t {
    for (UIView *subview in self.alertView.subviews)
    {
		if ((![subview isKindOfClass:[UIProgressView class]] &&
			 ![subview isKindOfClass:[UILabel class]])) {
			CGRect frame = subview.frame;
			CGRect nframe = CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, frame.size.height);
